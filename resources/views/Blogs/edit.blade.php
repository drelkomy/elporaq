...
<div class="container">
    ...
    <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return updateContent()">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <div id="editor" class="form-control @error('content') is-invalid @enderror" required></div>
            @error('content')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <!-- حقل مخفي لتخزين المحتوى من Quill -->
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('tags') is-invalid @enderror" id="tags" name="tags" value="{{ old('tags', implode(',', $blog->tags->pluck('name')->toArray())) }}" placeholder="مثال: Laravel, PHP, Web Development">
            @error('tags')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <!-- زر التحديث -->
        <button type="submit" class="btn btn-primary">تحديث التدوينة</button>
    </form>
</div>
